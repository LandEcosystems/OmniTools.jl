...
function plotMap(name_extensions, var_name, out_path, t_index; colormap = :plasma)
    ...
    if length(ds_dims) == 4
        ...
        for k in 1:min(l_dim, 8)
            if k ==1
                plt = heatmap!(axs[k], ds_tmp[k, t_index, :, :]; colormap) #  colorrange=(0,100)
            else
                plt = heatmap!(axs[k], ds_tmp[k, t_index, :, :]; colormap)
            end
...
#     plotMap(name_extensions, "auto_respiration_f_airT", out_path_maps, 1; colormap=:haline)
# end
