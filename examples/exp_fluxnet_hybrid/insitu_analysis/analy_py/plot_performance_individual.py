...
hp=0.5
ym=0.243
f_dir = './model_performance_maps_wroasted/individual'
...
for forcing in forc_order:
    ...
    for version in fn_versions:
        ...
        with open(f"perf_summary_wroasted_sets/performance_summary_{forname}_{version}.pkl", "rb") as tf:
            data_met_a = pickle.load(tf)
        for _set in ssets:
            data_met = data_met_a[_set]
            ...
            for _freq in time_freqs:
                ...
                for _met in metrs:
                    ...
                    for _var in varib_list:
                        ...
                        print(f'{_var}, {_freq}, {_met}, {_set}:: plotting')
                        dat = data_met[_freq][_met][_var]
                        dat[np.isinf(dat)] = np.nan
                        if len(dat) == sum(np.isnan(dat)):
                            print(f'{_var}, {_freq}, {_met}, {_set}:: all nans')
                            continue
                        ...
                        if _met == 'mef':
                            ...
                            # plt.xlim(0,1)
                        ncolo=50
                        # dmin = np.nanpercentile(dat,5)
                        # dmax = np.nanpercentile(dat,95)
                        bounds = np.linspace(dmin,dmax,ncolo * 2,endpoint=True)
                        c_list = get_colomap(cmap_n, bounds)
                        cm2 = mpl.colors.ListedColormap(clist) # for rainfall
                        # cm2 = cmap_n
                        ...
                        # xu.rem_axLine()
                        plt.title('{vname} [{nsite} sites], {_met}, {_freq}, {_set}'.format(letter=string.ascii_letters[spn-1],nsite=sum(~np.isnan(dat)),vname=varibs[_var]['name'], _met=_met, _freq=_freq, _set=_set),fontsize=6,y=0.94)
                        cb=plt.colorbar(orientation='horizontal',pad=0.01, aspect=35,shrink=0.7,extend='both')
                        fix_cb(cb,ax_fs=7)
                        ax = plt.axes([ax_l[0]+0.03,ax_l[1]+0.135,0.06,0.07])
                        ...
                        lt0 = (tmp1 < bmin).sum()*100/len(tmp1)
                        if ~np.isnan(lt0):
                            lt0 = int(lt0)
                        lt05 = (tmp1 < bmin + bmax/2).sum()*100/len(tmp1)
                        if ~np.isnan(lt05):
                            lt05 = int(lt05)
                        ...
                        if _met == 'mef':
                            txt_m=f'<{round(bmin, 2)}: {lt0}%, <{round(bmin + bmax/2, 2)}: {lt05}%\n[{q25}, {q50}, {q75}]'
                        else:
                            txt_m=f'<{round(bmin, 2)}: {lt0}%, <{round(bmin + bmax/2, 2)}: {lt05}%\n[{q25}, {q50}, {q75}]'
                            # txt_m=f'<0.5: {lt05}%\n[{q25}, {q50}, {q75}]'
                            
                        # plt.title(txt_m,fontsize=4,y=0.962)
                        # plt.xlim(np.nanpercentile(dat,25), np.nanpercentile(dat,75))
                        # plt.xlim(bmin, bmax)
                        # if _met == 'mef':
                        #     plt.xlim(-1,1)
                        # elif _met == 'r2':
                        #     plt.xlim(0,1)
                        # else:
                        #     print("no x-axis limit")
                        dat_tmp = dat.copy()
                        dat_tmp[dat>bmax] = bmax
                        dat_tmp[dat<bmin] = bmin
                        # dat_tmp[dat>np.nanpercentile(dat,98)] = np.nan
                        # dat_tmp[dat<np.nanpercentile(dat,2)] = np.nan
                        plt.ylim(0,0.4)
                        plt.hist(dat_tmp,bins=10,alpha=0.65,weights=np.ones(len(dat)) / len(dat),rwidth=0.8,color='#3399ff')
                        xu.ax_clrXY(axfs=4)
                        # plt.title(_var)
                        spn=spn+1
                    
                        plt.savefig(os.path.join(f_dir,f'{_met}_{_freq}_{forname}_{version}_{_var}_{_set}.png'), dpi=450,
                            bbox_inches='tight')
                        plt.close()
